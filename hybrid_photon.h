 ...
 public:
  static void integrate_T(Fragment *fragment, Hydro *hydro, double factor, double Tc){
   ...
   if((tau+tauplus)/2<0.6){
	...
	// if(first){
	 // cout<<" initial x="<<(x+xplus)/2<<" initial y="<<(y+yplus)/2<<" initial z="<<(z+zplus)/2<<" initia tau="<<(tau+tauplus)/2<<" vx="<<vx<<" vy="<<vy<<" vz="<<vz<<" gamma="<<gamma<<" vdotw="<<wdotv<<" vdotv="<<vdotv<<" xF step="<<coef<<endl;
	 // first=false;
	// }
    if(quench_method==0){
   ...
   // cout<<"quenched E="<<E<<" xF="<<tplasma<<" final T="<<T<<" final x="<<x<<endl<<" final y="<<y<<" final z="<<z<<endl;
...
struct DataFile_Parser
{
 ...
 { 
  ...
  while(getline(input_file,line))
  {
   ...
   if(state==1 and string::npos == line.find("0.000123"))
   {
    int id;
    double px;
    double py;
   ...
   else if(state==1 and string::npos != line.find("0.000123"))
   {
    number_of_events++;
    // getline(input_file,line);
    // cout<<"changing event"<<endl;
